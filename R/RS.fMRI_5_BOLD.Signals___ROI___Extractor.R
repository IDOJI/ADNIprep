RS.fMRI_5_BOLD.Signals___ROI___Extractor = function(path_Sub, Pipeline = "FunImgARCWSF", Atlas = "AAL3", path_Export){
  ...
  Results = sapply(seq_along(path_Sub_List), function(i, ...){
    ...
    path_Export_ROI.Signals = paste0(path_Export, "/", Pipeline, "___ROI.Signals")
    dir.create(path_Export_ROI.Signals, showWarnings = F)
    path_Export_Correlation = paste0(path_Export, "/", Pipeline, "___Correlation")
    dir.create(path_Export_Correlation, showWarnings = F)
    path_Export_FisherZ = paste0(path_Export, "/", Pipeline, "___FisherZ")
    dir.create(path_Export_FisherZ, showWarnings = F)


    # Exporting
    saveRDS(object = ith_ROI_Signals, file = paste(path_Export_ROI.Signals, filename_ROI_Signals, sep="/"))
