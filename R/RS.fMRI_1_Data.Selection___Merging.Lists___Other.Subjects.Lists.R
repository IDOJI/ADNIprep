RS.fMRI_1_Data.Selection___Merging.Lists___Other.Subjects.Lists = function(Merged_VISCODE.list,
  ...
  Merged_Full.list = lapply(seq_along(Merged_VISCODE.list), function(i, ...){
    ...
    ith_Merged_1.df = merge(ith_RID.df, ith_ADNIMERGE.df, by = cols_intersection, all = T) %>% relocate(ADNIMERGE___EXAMDATE, .after = VISCODE2)
    ...
    ith_Merged_5.df = merge(ith_Merged_4.df, ith_MMSE.df, by = cols_intersection, all = T) %>% relocate(ends_with("DATE"), .after = ADNIMERGE___EXAMDATE)
