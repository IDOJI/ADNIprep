RS.fMRI_1.2_Merging.Lists___Merging.By.Date.Having.Both.Images = function(data.list=Intersect_by_RID.list, what.date){
  ...
  # Change date name
  #=============================================================================
  df1_SQ = df1_SQ %>% dplyr::rename(STUDY.DATE_SEARCH = STUDY.DATE)
  df2_SQ = df2_SQ %>% dplyr::rename(STUDY.DATE_QC = SERIES_DATE)
  df3_SQ = df3_SQ %>% dplyr::rename(STUDY.DATE_NFQ = SCANDATE)
  ...
  df1_SQ = df1_SQ %>% rename(IMAGE_ID_SEARCH = IMAGE_ID)
  df1_SQ = df1_SQ %>% rename(RID_SEARCH = RID)
  ...
  df2_SQ = df2_SQ %>% rename(IMAGE_ID_QC = IMAGE_ID)
  df2_SQ = df2_SQ %>% rename(RID_QC = RID)
  df2_SQ = df2_SQ %>% rename(FIELD_STRENGTH_QC = FIELD_STRENGTH)

  df3_SQ = df3_SQ %>% rename(RID_NFQ = RID)
