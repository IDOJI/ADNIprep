RS.fMRI_4.2_Removing.Specific.Files = function(path,
  ...
  lapply(path, FUN=function(ith_path, ...){
    ...
    if(!is.null(recursive_folder) & length(path) == length(recursive_folder)){
      RS.fMRI_4.2_Removing.Specific.Files___Single.Path(ith_path, recursive_folder[ind], files_list, remove)
    }else if(is.null(recursive_folder)){
      RS.fMRI_4.2_Removing.Specific.Files___Single.Path(path = ith_path, files_list = files_list, remove = remove)
    }
